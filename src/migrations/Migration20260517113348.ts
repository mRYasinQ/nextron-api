...
export class Migration20260517113348 extends Migration {
  ...
  override down(): void | Promise<void> {
    this.addSql(`pragma foreign_keys = off;`);
    this.addSql(`create table \`sessions__temp_alter\` (\`browser\` text not null, \`created_at\` datetime not null, \`expire_at\` datetime not null, \`id\` numeric not null primary key, \`os\` text not null, \`token\` text not null, \`updated_at\` datetime not null, \`user_id\` numeric not null, constraint \`sessions_user_id_foreign\` foreign key (\`user_id\`) references \`users\` (\`id\`));`);
